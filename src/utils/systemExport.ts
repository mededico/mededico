...
// Función para obtener el código fuente completo de AdminContext
...
// Reducer implementation with all current state
...
export function CheckoutModal({ isOpen, onClose, onCheckout, items, total }: CheckoutModalProps) {
  ...
  const generateOrderText = () => {
    ...
    orderText += \`🚚 Entrega (\${deliveryZone.split(' > ')[2] || 'Zona seleccionada'}): +$\${deliveryCost.toLocaleString()} CUP\\n\`;
                    ...
                    {deliveryCost > 0 && (
                      <div className="mt-3 p-4 bg-gradient-to-r from-green-50 to-blue-50 rounded-xl border border-green-200">
                        ...
                        <div className="text-xs text-green-600 ml-11">
                          ✅ Zona: \${deliveryZone.split(' > ')[2] || 'Zona seleccionada'}
                        </div>
                      </div>
                    )}
                  </div>
